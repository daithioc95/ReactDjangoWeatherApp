...
class LocationCard extends React.Component {
	...
	// Funciton to post favourite to user when bookmark icon clicked
	setIsFav = () => {
		...
		if (localStorage.getItem("user")) {
			...
			this.interval = setTimeout(() => {
				// API call which passes location and gets weather data
				axios
					.post(`${API_ENDPOINT}getuserfavs/`, {
						params: {
							id: CardId,
							add: this.state.isFav,
	...
	render() {
		// if from search and not on dash we need to render the modal only
		if (this.props.fromSearch && !this.props.onDash) {
			return (
				<ResultModal
					infoButton={
						this.props.onDash ? "info-button-loc" : "info-button"
					}
					onClose={this.showModal}
					show={this.state.show}
					resultData={this.state.details}
					favourited={this.state.isFav}
					updateFave={this.setIsFav}
					dashAdd={true}
			...
			return (
				<div className="col-lg-4 col-md-6 col-sm-1 locationCards">
					{/* Map cards depending on data returned from API */}
								...
								<div className="">
									<div className="card">
										<BsFillBookmarkStarFill
											onClick={() => this.setIsFav()}
											locationid={city_weather.id}
											size={45}
											className={
												this.state.isFav
													? "fav-item-icon bookmark-icon"
													: "bookmark-icon"
											}
										/>
										{/* show add to dashboard icon only when on homepage */}
